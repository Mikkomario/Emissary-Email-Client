...
object SubjectData extends FromModelFactoryWithSchema[SubjectData]
{
	...
	override protected def fromValidatedModel(valid: Model) = SubjectData(valid("created").getInstant)
}
...
/**
  ...
  */
case class SubjectData(created: Instant = Now) extends SubjectFactory[SubjectData] with ModelConvertible
{
	// IMPLEMENTED	--------------------
