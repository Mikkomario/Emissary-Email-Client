package vf.emissary.database.access.many.text.placement.statement
...
/**
  * A common trait for access points which target multiple statement placements or similar instances at a time
...
trait ManyStatementPlacementsAccessLike[+A, +Repr] extends ManyTextPlacementsAccessLike[A, Repr]
{
	...
	/**
	  ...
	  * @return Copy of this access point that only includes statement placements 
	...
	/**
	  ...
	  * @return Copy of this access point that only includes statement placements where statement id is within the specified value set
	  */
