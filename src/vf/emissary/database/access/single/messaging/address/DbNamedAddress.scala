...
import vf.emissary.database.storable.messaging.{AddressDbModel, AddressNameDbModel}
import vf.emissary.model.combined.messaging.NamedAddress

/**
  * Used for accessing individual named addresses
...
object DbNamedAddress extends SingleModelAccess[NamedAddress] with UnconditionalView with Indexed
{
	...
	/**
	  * @param
	  * 
		 condition Filter condition to apply in addition to this root view's condition. Should yield unique named addresses.
	  * @return An access point to the named address that satisfies the specified condition
	  */
	private def distinct(condition: Condition) = UniqueNamedAddressAccess(condition)
}
