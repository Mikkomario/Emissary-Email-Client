...
import vf.emissary.database.model.messaging.{MessageModel, MessageThreadModel, MessageThreadSubjectLinkModel}
...
object DbMessageThread extends SingleRowModelAccess[MessageThread] with UnconditionalView with Indexed
{
	 ...
	 * @param senderId Id of the address that must be involved in the returned thread
	 * @param connection Implicit DB connection
	 * @return Id of the most recent thread that fulfills the specified requirements.
	 *         None if no such thread exists.
	 */
	def findIdForSentSubject(subjectId: Int, senderId: Int)(implicit connection: Connection) = {
		// Subject must match
		...
		val addressCondition = messageModel.withSenderId(senderId).toCondition
		findColumn(index,
