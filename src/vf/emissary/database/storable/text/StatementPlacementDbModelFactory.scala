...
import vf.emissary.database.props.text.{StatementPlacementDbProps, StatementPlacementDbPropsWrapper}
...
object StatementPlacementDbModelFactory
{
	...
	/**
	  * Used for constructing StatementPlacementDbModel instances and for inserting statement placements to the database
	  * @param table Table targeted by these models
	  * @param statementPlacementDbProps Properties which specify how the database interactions are performed
		...
		extends StatementPlacementDbModelFactory with StatementPlacementDbPropsWrapper
	{
		...
		override protected def complete(id: Value, 
			data: StatementPlacementData) = StatementPlacement(id.getInt, data)
